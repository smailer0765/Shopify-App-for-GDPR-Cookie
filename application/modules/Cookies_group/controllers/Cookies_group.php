...
class Cookies_group extends MX_Controller{
	...
	function get_cookies_group()
	{
		...
		foreach($rResult->result_array() as $aRow) {
			...
			$row[] = '<div class="hidden-sm hidden-xs action-buttons">
						<a class="green" href="'.base_url().'cookies_group/edit_cookies_group/'.$aRow['id'].'">
							<i class="ace-icon fa fa-pencil bigger-130"></i>
						</a>
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<a class="red delete_link" href="'.base_url().'cookies_group/delete_cookies_group/'.$aRow['id'].'" onclick="'.$delete.'">
	...
	function create_new_cookies_group()
	{
		...
		if($flag != false){
			$this->set_success("New Cookies Group created successfuly!");
		}else{
			$this->set_error("ERROR! Unknown Error!");
		}
		redirect("cookies_group");
	}
	
	function delete_cookies_group($id = "")
	{
		if ($id != ""){
			$filter = array("id" => $id);
			$table = "cookie_groups";
			...
			if ($data != false){
				...
				if ($flag != false){
					$this->set_success("Cookies Group Deleted successfuly!");
				}else{
					$this->set_error("ERROR! Unknown Error!");
				}
			}else{
	...
	function edit_cookies_group($id = "")
	{
		if($id != ""){
	...
	function update_cookies_group($id = "")
	{
		if($id != "") 
		{
			...
			if($flag != false){
				...
				if($flag != false){
					$this->set_success("Cookies Group updated successfuly!");
				}else{
					$this->set_error("ERROR! While updating Cookies Group!");
				}
			}
		}
		redirect("cookies_group");
